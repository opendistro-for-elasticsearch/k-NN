 ...
 */
class KNN84DocValuesConsumer extends DocValuesConsumer implements Closeable {

    private final Logger logger = LogManager.getLogger(KNN84DocValuesConsumer.class);
    ...
    public void addKNNBinaryField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        if (field.attributes().containsKey(KNNVectorFieldMapper.KNN_FIELD)) {
            ...
            KNNCodecUtil.Pair pair = KNN84Codec.getFloats(values);
        ...
        try {
            ...
            for (FieldInfo fieldInfo : mergeState.mergeFieldInfos) {
                ...
                if (type == DocValuesType.BINARY) {
                    addKNNBinaryField(fieldInfo, new KNN84DocValuesReader(mergeState));
                }
            }
        } catch (Exception e) {
